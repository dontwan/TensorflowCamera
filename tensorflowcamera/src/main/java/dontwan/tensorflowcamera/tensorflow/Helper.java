package dontwan.tensorflowcamera.tensorflow;
...
public class Helper {
    ...
    //private static final int MAX_BEST_RESULTS = 10;
    //private static final float RESULT_CONFIDENCE_THRESHOLD = 0.35f; // Minimal score for the list of recognition

    public static String[] readLabels(Context context) {

        AssetManager assetManager = context.getAssets();
        ArrayList<String> result = new ArrayList<>();
        try (InputStream is = assetManager.open(TensorFlowCamera.getLabelsFile());
            ...
            throw new IllegalStateException("Cannot read labels from " + TensorFlowCamera.getLabelsFile());
        }
    }

    public static List<Classifier.Recognition> getBestResults(float[] confidenceLevels, String[] labels) {
        // Find the best classifications.
        PriorityQueue<Classifier.Recognition> pq = new PriorityQueue<>(TensorFlowCamera.getMaxBestResults(),
        ...
        for (int i = 0; i < confidenceLevels.length; ++i) {
            if (confidenceLevels[i] > TensorFlowCamera.getResultConfidenceThreshold()) {
        ...
        int recognitionsSize = Math.min(pq.size(), TensorFlowCamera.getMaxBestResults());
